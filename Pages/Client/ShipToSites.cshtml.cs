    ...
    public class ShipToSitesModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            ...
            if (registration == null)
            {
                ...
                ShipToSites = new List<ShipToSite>(); // Clear the list to avoid any null reference issues
                return Page();
            }
...
                // Log and return error if AgencyRegistrationId is still not set
                if (site.AgencyRegistrationId == 0)
                {
                    _logger.LogError("AgencyRegistrationId is required.");
                    ModelState.AddModelError("AgencyRegistrationId", "The AgencyRegistrationId field is required.");
